...
public class FrameSendBox extends javax.swing.JFrame {
    ...
    public FrameSendBox(String eposta) {
        initComponents();
        setLocationRelativeTo(null);

        user = controllerUser.Getir(eposta);
        mail = eposta;
        setTitle("Giden Kutusu >> " + user.getAd() + " " + user.getSoyad());
        dosyaAdi = "SentMailBox (" + user.getKullaniciId() + ").txt";
        Listele();
    }

    public void Listele() {
        if (controllerMail.Liste(dosyaAdi) != null) {
            tbMailler.setModel(controllerMail.TabloDoldur(true, tbMailler, controllerMail.Liste(dosyaAdi)));
        }
    }
    ...
    private void btnSilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSilActionPerformed
        ...
        try {
            String alanEposta = tbMailler.getValueAt(tbMailler.getSelectedRow(), 0).toString();
            TypeBasicUser alan = controllerUser.Getir(alanEposta);
            mail.setGonderen(user);
            mail.setAlan(alan);
