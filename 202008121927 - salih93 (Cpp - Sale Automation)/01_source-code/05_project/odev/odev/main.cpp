...
void HakedisKullanilanUrunler() {
	int hakedisNo=0,arananNo = 0;

	while (1) {
		cout << "Hakedis No : "; cin >> arananNo;
		if (HakedisKontrol(arananNo) == 0)
			cout << "Hakedis bulunamadi !" << endl;
		else
			break;
	}
	HakedisYazdir(arananNo);

	int urunNo, oncekiUrunNo=0;
	int urunAdet;
