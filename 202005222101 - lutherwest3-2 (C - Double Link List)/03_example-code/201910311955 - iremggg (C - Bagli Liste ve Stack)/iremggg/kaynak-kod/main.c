...
void menu(){
	...
	switch(secim){
		...
		case 4: // node(dugum) arama islemi
			printf("\nArama icin deger giriniz > ");
			int aranacak;
			scanf("%d",&aranacak);
			if(search(tree,aranacak) == 1){
				printf("\n>> Aranan %d degeri %d. asamada bulundu.",aranacak,step);
			}
			else
				printf("\n>> Aranan %d bulunamadi!",aranacak);
			menu();
			break;
		case 5: //node(dugum) silme islemi
			printf("\nSilmek icin deger giriniz > ");
			int silinecek;
			scanf("%d",&silinecek);
			...
			if(search(tree,silinecek) == 1){
				...
				printf("\n>> Silinecek %d degeri bulunamadi!",silinecek);
			menu();
			break;
		case 0: //cikis islemi
			exit(1);
			break;
		default: //beklenen disinda deger girilmesi durumu
...
//bagli liste(linklist) icerisindeki minumum deger bulmak icin fonksiyon
int min(Node *tree){
...
//bagli liste(linklist) icerisindeki maximum deger bulmak icin fonksiyon
int max(Node *tree){
